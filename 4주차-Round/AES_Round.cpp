...
// AES의 암호화 과정
// state[16] (평문) --> [암호화 과정: 업데이트] --> state[16] (암호문)
// AES Encryption
// AddRoundKey

// (Round function) -- 9번 반복
//     SubBytes
//     ShiftRows
//     MixColumns
//     AddRoundKey
...
void AES_Round(byte state[16], byte RK[16]) {
...
void AES_Inv_Round(byte state[16], byte RK[16]) {
    print_AES_state(state, "Round IN = ");
        AddRoundKey(state, RK);
    print_AES_state(state, "After AddRoundKey = ");
    InvMixColumns(state);
...
void test_AES_round() {
    byte PT[16] = { 0x19, 0x3d, 0xe3, 0xbe, 0xa0, 0xf4, 0xe2, 0x2b, 
                    0x9a, 0xc6, 0x8d, 0x2a, 0xe9, 0xf8, 0x48, 0x08 };
                    
    byte RK[16] = { 0xa0, 0xfa, 0xfe, 0x17, 0x88, 0x54, 0x2c, 0xb1,
